...
plot_df$celltype2 = plot_df$celltype
plot_df[plot_df$celltype2 %in%c('Spinal cord','Paraxial mesoderm'),'celltype2']=NA
ggplot(plot_df,aes(x=DiffusionMap_1,y=DiffusionMap_2,color=celltype2))+
  geom_point(size=1.2)+scale_color_manual(values = fate_col)+
  theme_void()+theme(text = element_text(size=16))+theme(legend.position = "none")
ggsave('/data/jiangjunyao/polyATAC/figure_after_science/single cell fig/NMP_fate_analysis/embedding/NMP diffusion map embedding celltype_nodownstream.pdf',
       width = 10,height = 8)
